...
- Define **D** as $D_{i j}=\begin{array}{ll}{\sum_{k} B_{i k},} & {\text { if } i=j} \\ {0} & {\text { otherwise }}\end{array}$, then 

$$\sum_{i, j \in c_{x}} B_{i j}=\sum_{i, j \in c_{x}} D_{i j}=\sum_{i, j \in c_{x}} D_{i j} s_{i} s_{j}=\sum_{i, j \in c_{x}}\left(s_{i} s_{j} \delta_{i j} \sum_{k \in c_{x}} B_{i k}\right)$$ 
